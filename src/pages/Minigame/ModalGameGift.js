import gitboxImage from 'assets/bw_image/GIFTBOX_text 2.svg';
import giftInsiteImg from 'assets/bw_image/insiteGift.svg';
...
const ModalGame = ({ open, onClose, setIsOpenedGiftBox, setOpenPresent, openPresent }) => {
    ...
    const handleOpenPresent = (e) => {
        const eleGiftBoxParent = e.target.parentNode;
        eleGiftBoxParent.classList.add('bw_giftOpen');
        setIsOpenedGiftBox(true);
        setTimeout(() => {
            getVoucher().then((res) => {
                setVoucher(res.data);
                setOpenPresent(true);
    ...
    const genGiftBox = useCallback((total) => {
        const arr = [];
        while (total > 0) {
            arr.push(total);
            total--;
    ...
    return (
        <Fragment>
            <div className="popup" id="popup-game">
                ...
                <div className="content-giftbox">
                    <div className="genBox">
                        {genGiftBox(12).map((item) => {
                            return (
                                <div
                                    key={item}
                                    className="bw_giftBox bw_giftBox__rp"
                                    onClick={handleOpenPresent
                                    
                                    }
                                >
                                    <div className="bw_lidBox"></div>
                                    <div className="bw_giftInsite">
                                        <img src={giftInsiteImg} alt="2" />
                                    </div>
                                    <div className="bw_bodyBox"></div>
                                </div>
                            );
                        })}

                        {/* <li onClick={() => handleOpenPresent()}>
                            <img src={boxImage} width="100%" alt="" />
                        </li> */}
                    </div>
